...
/**
 * Quiz Builder section for Educator portal.
 * @returns {JSX.Element} QuizBuilderSection component.
 */
const QuizBuilderSection = () => {
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      ...
      <form onSubmit={handleQuizSubmit} className="bg-dark-card border border-gray-800 p-6 rounded-xl space-y-4">
        <div>
          <label className="block text-sm text-gray-400 mb-1">Question Text</label>
          <input required type="text" value={formData.text} onChange={e=>handleInputChange(e, 'text')} className="w-full bg-dark-bg border border-gray-700 py-2 px-3 rounded text-white focus:border-green focus:outline-none" />
        </div>
        <div className="grid grid-cols-2 gap-4">
          {[0,1,2,3].map(i => (
            <div key={i}>
              <label className="block text-sm text-gray-400 mb-1">Option {i + 1}</label>
              <input required type="text" value={formData[`opt${i}`]} onChange={e=>handleInputChange(e, `opt${i}`)} className="w-full bg-dark-bg border border-gray-700 py-2 px-3 rounded text-white focus:border-green focus:outline-none" />
            </div>
          ))}
        </div>
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input required type="number" min="0" max="3" value={formData.correct} onChange={e=>handleInputChange(e, 'correct')} className="w-full bg-dark-bg border border-gray-700 py-2 px-3 rounded text-white focus:border-green focus:outline-none" />
          </div>
          <div>
            <label className="block text-sm text-gray-400 mb-1">Difficulty</label>
            <select value={formData.diff} onChange={e=>handleInputChange(e, 'diff')} className="w-full bg-dark-bg border border-gray-700 py-2 px-3 rounded text-white focus:border-green focus:outline-none">
        ...
        <div>
          ...
          <textarea required value={formData.expl} onChange={e=>handleInputChange(e, 'expl')} className="w-full bg-dark-bg border border-gray-700 py-2 px-3 rounded text-white focus:border-green focus:outline-none h-24"></textarea>
        </div>
